...
export function DerivationSteps({ steps }: DerivationStepsProps) {
  if (!steps.length) {
    return (
      <div className="panel-muted p-6 text-sm text-slate-400">
        Run the solver to display the derivation steps here.
      </div>
    );
  }
  ...
  return (
    <div className="panel space-y-4 p-6">
      <div>
        <span className="tag">Teaching mode</span>
        <p className="text-sm text-slate-400">Balance equations laid out step by step</p>
      </div>
